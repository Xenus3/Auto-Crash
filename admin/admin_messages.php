...
$update_status = $DB->prepare("UPDATE demandes_contact SET status = ? WHERE id_demande_contact = ?");
$update_status->execute([$payment_status, $order_id]);
}

if(isset($_GET['delete'])){
$delete_id = $_GET['delete'];
$delete_order = $DB->prepare("DELETE FROM demandes_contact WHERE id_demande_contact = ?");
...
<?php 
$select_orders = $DB->prepare("SELECT * FROM demandes_contact natural join utilisateurs");
...
<p>Commentaire : <span><?= $fetch_orders['commentaire']; ?></span></p>




 <form action="" method="POST">
<input type="hidden" name="order_id" value="<?= $fetch_orders['id_demande_contact']; ?>">
...
<div class="flex-btn">
    ...
    <a href="admin_messages.php?delete=<?php echo $fetch_orders['id_demande_contact']?>" class="delete-btn" onclick="return confirm('Voulez-vous supprimer la commande ?');">Supprimer</a>
</div>
